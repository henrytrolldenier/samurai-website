...
```
.
├── index.html          Landing page (kabuto 3D hero, tunnel, thesis + theme cards)
├── code.html           Theme I  · The Code
├── warrior.html        Theme II · The Warrior (katana 3D showcase)
...
│                       parallax, global sakura particles, and all 3D model scenes
│
├── models/             3D assets (GLB)
│   ├── pbr_kabuto_samurai_helmet.glb     loaded on index.html
│   ├── dragon_katana_oni_koroshi.glb     loaded on warrior.html
...
> The `models/` folder must contain the three GLB files with **exactly** these names. The
> pages reference them by relative path, so the names and folder are case sensitive on GitHub
> Pages.
...
* **Three GLB models** rendered with Three.js `GLTFLoader`, full PBR lighting via
  `PMREMGenerator`, ACES Filmic tone mapping, sRGB output, and soft shadows.
  * **Kabuto helmet** floats and slowly rotates in the landing hero under a deep red
    `RectAreaLight` above and a warm gold `PointLight` below.
  * **Katana** drifts up and down at a 15 degree tilt on the Warrior page, wrapped in a red
    bloom glow.
  * **Cherry blossom branch** rotates in the corner of the Culture header while individual
    petals fall around it.
  * Every model is click and drag rotatable via `OrbitControls`.
...
* Three.js **r128** from cdnjs, with `GLTFLoader`, `OrbitControls`, and
  `RectAreaLightUniformsLib` from the matching `three@0.128.0` example bundles on jsDelivr.
* Fonts: **Cinzel** for headings and **Crimson Text** for body, served by Google Fonts.
* If a GLB fails to load, the page falls back to a lacquered placeholder mesh so nothing breaks.
